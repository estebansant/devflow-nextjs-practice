...
const LeftSidebar = () => {
  ...
  return (
    <nav className="background-light900_dark200 light-border custom-scrollbar sticky left-0 top-0 flex h-screen flex-col justify-between overflow-y-auto border-r p-6 pt-36 shadow-light-300 dark:shadow-none max-sm:hidden lg:w-[266px]">
        ...
        {sidebarLinks.map((item) => {
          ...
          return (
            <Link
              href={item.route}
              key={item.route}
              className={`${isActive ? "primary-gradient rounded-lg text-light-900" : "text-dark300_light900"} flex w-fit items-center justify-start gap-4 bg-transparent p-4 lg:w-full`}
            >
              <Image
                src={item.imgURL}
                alt={item.label}
      ...
      <SignedIn>
        <SignOutButton>
          <div className="text-dark300_light900 flex h-fit items-center justify-center gap-4 bg-transparent p-6">
            <svg
              viewBox="0 0 24 24"
              xmlns="http://www.w3.org/2000/svg"
              className="size-6 min-h-6 min-w-6 fill-dark-300 stroke-dark-300 dark:fill-light-700 dark:stroke-light-700"
            >
            ...
            <p className="base-medium text-dark300_light900 w-full max-lg:hidden">
              Logout
            </p>
          </div>
        </SignOutButton>
